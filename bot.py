...
class ChorchaQuizBot:
    def __init__(self):
        ...
        self.password = os.environ.get("CHORCHA_PASS", "00")
        self.name = os.environ.get("CHORCHA_NAME", "00")
        self.ac = os.environ.get("CHORCHA_AC", "00")
    ...
    def send_telegram_report(self, success: bool, error_msg: str = None, traceback_str: str = None):
        ...
        name_esc = html.escape(self.name)
        ac_esc = html.escape(self.ac)
        ...
        details_part = (
            ...
            f"👤 <b>NAME:</b> <code>{name_esc}</code> | <b>AC:</b> <code>{ac_esc}</code>\n"
        )
    ...
    def update_github_secret(self, cookies) -> bool:
        ...
        gh_pat = os.environ.get("GH_PAT") or os.environ.get("GH_TOKEN") or os.environ.get("PAT")
        if not gh_pat or not gh_pat.strip():
            logger.info("GitHub Personal Access Token (GH_PAT/GH_TOKEN/PAT) not found in environment. Skipping secret update.")
            return False
        ...
        try:
            ...
            repo = os.environ.get("GITHUB_REPOSITORY", "streakaliver/jihad")
            ...
            if result.returncode == 0:
                ...
                logger.info(f"Failed to update GitHub Secret. gh CLI exit code: {result.returncode}")
                logger.info(f"gh CLI stderr: {result.stderr}")
                logger.info(f"gh CLI stdout: {result.stdout}")
                return False
        except Exception as e:
            logger.info(f"Failed updating GitHub Secret via CLI exception: {e}")
            return False
    ...
    def execute_pipeline(self):
        ...
        with sync_playwright() as playwright:
            headless_mode = os.environ.get("HEADLESS", "true").lower() == "true" or "CI" in os.environ
